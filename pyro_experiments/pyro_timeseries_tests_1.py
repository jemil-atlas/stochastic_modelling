...
# i) Simple model: known trend functions, unknown noise level

def model(y_obs = None):
    coeffs = pyro.param("coeffs", init_tensor = torch.zeros(true_params.shape))
    sigma = pyro.param("sigma", init_tensor = torch.ones(1))
    mu_mat = mu_fun(x, coeffs)
    y_dist= pyro.distributions.Normal(mu_mat, sigma).to_event(1)
