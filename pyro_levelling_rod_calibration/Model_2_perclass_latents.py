...
We go for a simple hierarchical bayesian model: A latent variable pair (offset, scale)
for each rod class. This means that a posterior distribution over the params is 
learned that assumes random rod construction but perfect measurement accuracy.
...
Dataset: minimal
Model: latent vars for each rod class
