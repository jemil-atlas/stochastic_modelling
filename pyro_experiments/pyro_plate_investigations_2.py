...
different distribution shapes. In this second script, there also exists a nonzero
event_shape.
...
x_data = torch.tensor(np.random.normal(mu_true,sigma_true, full_shape)) 
...
#                       ii) to_event(3) to declare dependence
#
...
# to_event(3), the distribution's 3 rightmost dimensions are assumed as a single
#  event. d contains [10,2,5] independent copies of the Normal but considered as 
# one event -> d.batch shape is () and d.event_shape is [10,2,5]
pyro.clear_param_store()
def model(x_obs = None):
    mu = pyro.param("mean",torch.tensor(2.0))
    d = dist.Normal(mu,1).expand([10,2,5]).to_event(3)
    x = pyro.sample("x",d, obs = x_obs)
    # print( "batch_shape : ",  d.batch_shape, "event_shape : ", d.event_shape)
    return x
guide = pyro.infer.autoguide.AutoNormal(model)
...
#  -> d.batch shape is [10,2] and d.event_shape is [5]
pyro.clear_param_store()
def model(x_obs = None):
    mu = pyro.param("mean",torch.tensor(2.0))
...
# Define model & guide. Since we expand the distribution to [10,2,5] by multiplying
# with a ones tensor of that shape, the distribution d contains [10,2,5] independent
...
def model(x_obs = None):
    mu = pyro.param("mean",torch.tensor(2.0))
    extension_tensor = torch.ones([10,2,5])
    d = dist.Normal(mu*extension_tensor,1).to_event(1)
    with pyro.plate("plate_1", size = 2, dim = -1):
        with pyro.plate("plate_2", size = 10, dim = -2):
            x = pyro.sample("x",d, obs = x_obs)
    # print( "batch_shape : ",  d.batch_shape, "event_shape : ", d.event_shape)
...
def model(x_obs = None):
    mu = pyro.param("mean",torch.tensor(2.0))
    d = dist.Normal(mu,1).expand([10,2,5])
    with pyro.plate("plate_1", size = 2, dim = -2):
        with pyro.plate("plate_2", size = 10, dim = -3):
            x = pyro.sample("x",d, obs = x_obs)
    # print( "batch_shape : ",  d.batch_shape, "event_shape : ", d.event_shape)
    return x
guide = pyro.infer.autoguide.AutoNormal(model)
