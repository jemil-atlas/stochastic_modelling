...
data = loadmat('submatrix_collection_training_20x20_2023_2days.mat')
...
phase_vals_data = phase_vals_data[1000:1100,:]
info_mats_data = info_mats_data[1000:1100,:,:]
...
n_y = 20
n_total = n_x*n_y
shape_example_data = np.transpose(info_mats_data[:,:,0].reshape([-1,20,20]), (0,2,1)).shape
...
class TRIStochastics(pyro.nn.PyroModule):
    ...
    def __init__(self, list_inputs, dim_hidden, dim_l2, base_data):
        ...
        self.map_to_l2 = MapToL2(self.dim_arg, self.dim_hidden, self.dim_l2).double()
    ...
    def model(self, base_data, observations = None):
        ...
        with pyro.plate('batch_plate',size = self.n_samples, dim = -1, subsample_size = subsample_size) as ind:
            ...
            cov_regularizer = 1e-3*(torch.eye(n_total).repeat(subsample_size, 1, 1)).double()
            subsampled_observations = observations[ind] if observations is not None else None
            obs_dist = pyro.distributions.MultivariateNormal(loc = torch.zeros([subsample_size, n_total]).double(), covariance_matrix = cov_mats + cov_regularizer)
...
tri_stochastics = TRIStochastics(list_inputs, 5, 5, base_data)
...
def plot_realizations(base_data, full_data, **plotting_opts):
    ...
    base_data_slice = BaseData(torch.repeat_interleave((range_mats[0,:,:]).unsqueeze(0), subsample_size, dim = 0),
                               torch.repeat_interleave((azimuth_mats[0,:,:]).unsqueeze(0), subsample_size, dim = 0),
                               torch.repeat_interleave((x_mats[0,:,:]).unsqueeze(0), subsample_size, dim = 0),
                               torch.repeat_interleave((y_mats[0,:,:]).unsqueeze(0), subsample_size, dim = 0),
                               torch.repeat_interleave((z_mats[0,:,:]).unsqueeze(0), subsample_size, dim = 0),
                               torch.repeat_interleave((coherence_mats[0,:,:]).unsqueeze(0), subsample_size, dim = 0),
                               torch.repeat_interleave((aoi_mats[0,:,:]).unsqueeze(0), subsample_size, dim = 0),
                               torch.repeat_interleave((meanphase_mats[0,:,:]).unsqueeze(0), subsample_size, dim = 0),
                               torch.repeat_interleave((time_mats[0,:,:]).unsqueeze(0), subsample_size, dim = 0)
                               )
    ...
    for i in range(n_rows_plot):  # loop over rows of plot
        ...
        if i == 1:
            for j in range(n_realizations):
                ax = axs[i, j]
                ax.axis('off')
                image = phase_mats[scenario_index,:]
...
plot_realizations(base_data, full_data, n_realizations = 8)
