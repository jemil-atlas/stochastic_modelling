...
class TempTrainInfo():
    def __init__(self, x_data, vae, epoch_loss, model_trace, param_store):
        ...
        # # Initialize dictionaries to store top parameters and gradients
        # top_params = {}
        # top_grads = {}
        
        # # Extract and log top 5 parameters by magnitude from param store
        # n_extract = 1
        # for name, value in pyro.get_param_store().items():
        #     flat_params = torch.abs(value.detach()).flatten()
        #     top_indices = torch.topk(flat_params, n_extract).indices
        #     top_values = flat_params[top_indices].tolist()
        #     top_params[name] = top_values
    
        # # Extract and log top 5 gradients by magnitude from trace nodes
        # for name, site in model_trace.nodes.items():
        #     if site['type'] == 'param':
        #         flat_grads = torch.abs(site['value'].grad.detach()).flatten()
        #         top_indices = torch.topk(flat_grads, n_extract).indices
        #         top_values = flat_grads[top_indices].tolist()
        #         top_grads[name] = top_values
        # self.params_highest = top_params
        # self.grads_highest = top_grads
...
# iv) Investigate and optimize latent space
...
# latent_base_dist = pyro.distributions.MultivariateNormal(loc = torch.zeros([dim_z]), covariance_matrix = torch.eye(dim_z)).to_event(0)
# spline_transform = pyro.distributions.transforms.spline_coupling(dim_z, count_bins=8)
# latent_flow_dist = pyro.distributions.TransformedDistribution(latent_base_dist, [spline_transform])

# nf_steps = 5000
# dataset_nf = torch.tensor(latent_codes, dtype=torch.float)
# optimizer = torch.optim.Adam(spline_transform.parameters(), lr=5e-3)
# for step in range(nf_steps+1):
#     optimizer.zero_grad()
#     loss = latent_flow_dist.log_prob(dataset_nf).mean()
#     loss.backward()
#     optimizer.step()
#     latent_flow_dist.clear_cache()
#     if step % 500 == 0:
#         print('step: {}, loss: {}'.format(step, loss.item()))

# latent_codes_flow = latent_flow_dist.sample(torch.Size([1000,])).detach().numpy()


base_dist = pyro.distributions.MultivariateNormal(loc = torch.zeros([dim_z]), covariance_matrix = torch.eye(dim_z))

spline_transform = pyro.distributions.transforms.spline_coupling(dim_z, count_bins=8)
flow_dist = pyro.distributions.TransformedDistribution(base_dist, [spline_transform])

steps = 5000
dataset = latent_codes.clone().detach()
...
latent_codes_flow = flow_dist.sample(torch.Size([3000,])).detach().numpy()
latent_codes_flow_2d = latent_codes_flow[:,0:2]


    
# Simulate completely new data via learned multimodal distribution on latent space
...
# Plotting for the first panel
axes[0].hist2d(latent_codes_2d[:, 0], latent_codes_2d[:, 1], bins=(50, 50), cmap='viridis')
axes[0].set_xlabel('Dimension 1')
axes[0].set_ylabel('Dimension 2')
axes[0].set_title('2D Histogram of Latent Codes - Panel 1')
# Plotting for the second panel
axes[1].hist2d(latent_codes_flow_2d[:, 0], latent_codes_flow_2d[:, 1], bins=(50, 50), cmap='viridis')
axes[1].set_xlabel('Dimension 1')
axes[1].set_ylabel('Dimension 2')
axes[1].set_title('2D Histogram of Latent Codes - Panel 2')
