...
f_simu = torch.zeros([n_simu, n_t])

for k in range(n_simu):
    f_simu[k,:] = torch.tensor(np.random.multivariate_normal(scale(x[k])*mu, cov_mat))
...
def stoch_model(x, f_simu = None):                
    alpha = pyro.param('alpha', torch.ones([3]))
    scale = alpha[0] + alpha[1]*x + alpha[2]*x**2 
    ...
    with pyro.plate('x_simu',n_simu):
        sample = pyro.sample('obs', dist.MultivariateNormal(loc = mu*scale, covariance_matrix = sigma +0.01*torch.eye(n_t)),obs = f_simu)
        return sample
...
x_sample = torch.randint(low=1, high = 12, size = [1])
synth_simu = stoch_model(x_sample)
...
plt.plot(f_simu.T)
plt.plot(synth_simu.detach().numpy().T, color = 'r', linestyle = '--', linewidth = 3)
