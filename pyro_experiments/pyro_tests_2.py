...
Data is generated by a Normal distribution featuring unknown mean and variance.
...
def model(data=None):
    theta_1=pyro.param("theta_1",lambda:torch.ones(()),constraint=constraints.positive)
    theta_2=pyro.param("theta_2",lambda:torch.ones(()),constraint=constraints.positive)
    mu=theta_1
    sigma=theta_2
...
# pyro.render_model(const_mean_model, model_args=(), render_distributions=True, render_params=True)
